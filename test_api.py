...
check("解析性别", r["gender"] == "女")
check("解析省份", r["province"] == "北京市")
r = c.post("/v1/validate/idcard", json={"id_number": "110105194912310021"}).json()
check("校验位错误被拒", r["valid"] is False and "校验位" in r["reason"])
r = c.post("/v1/validate/idcard", json={"id_number": "110105209912310021"}).json()
check("未来生日被拒", r["valid"] is False and "晚于今天" in r["reason"])
...
check("合法号", r["valid"] is True and r["carrier"] == "中国移动")
